package digitalhouse.android.a0317moacns1c_02.Model.Media;

import java.io.Serializable;

/**
 * Created by forev on 17-Jun-17.
 */

public class Video implements Serializable {

    static final String YOUTUBE_VIDEO_URL = "http://www.youtube.com/watch?v=%1$s";
    static final String YOUTUBE_THUMBNAIL_URL = "http://img.youtube.com/vi/%1$s/0.jpg";
    static final String YOUTUBE_THUMBNAIL_FORMAT = "/0.jpg";
    private String id;
    private String iso_639_1;
    private String iso_3166_1;
    private String key;
    private String name;
    private String site;
    private Integer size;
    private String type;


    public String getYouTubeThumbnailURL(){
        return String.format(YOUTUBE_THUMBNAIL_URL, key);
    }

    public String getYoutubeVideoURL(){
       return String.format(YOUTUBE_VIDEO_URL, key);
    }

    public static String thumbnailToVideoURL(String thumbnailURL){
        Integer indexOfThumbnailThumbnailFormat = thumbnailURL.indexOf(YOUTUBE_THUMBNAIL_FORMAT);
        String key = thumbnailURL.substring(26, indexOfThumbnailThumbnailFormat);
        return String.format(YOUTUBE_VIDEO_URL, key);
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getIso_639_1() {
        return iso_639_1;
    }

    public void setIso_639_1(String iso_639_1) {
        this.iso_639_1 = iso_639_1;
    }

    public String getIso_3166_1() {
        return iso_3166_1;
    }

    public void setIso_3166_1(String iso_3166_1) {
        this.iso_3166_1 = iso_3166_1;
    }

    public String getKey() {
        return key;
    }

    public void setKey(String key) {
        this.key = key;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getSite() {
        return site;
    }

    public void setSite(String site) {
        this.site = site;
    }

    public Integer getSize() {
        return size;
    }

    public void setSize(Integer size) {
        this.size = size;
    }

    public String getType() {
        return type;
    }

    public void setType(String type) {
        this.type = type;
    }
}
